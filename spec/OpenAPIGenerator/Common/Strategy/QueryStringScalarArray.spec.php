...
describe(OAGC\Strategy\QueryStringScalarArray::class, function ()
{
	...
	context('->__construct', function ()
	{
		...
		it('throws on invalid type', function ()
		{
			...
			expect(static fn () => new OAGC\Strategy\QueryStringScalarArray(['type' => 'test']))->toThrow($exception);
		});
		it('throws on no format', function ()
		{
			$exception = new InvalidArgumentException('Unknown format "".');
			expect(static fn () => new OAGC\Strategy\QueryStringScalarArray(['type' => OAGC\Validator\QueryStringScalarArray::TYPE_INT]))->toThrow($exception);
		});
		it('throws on invalid format', function ()
		{
			$exception = new InvalidArgumentException('Unknown format "test".');
			expect(static fn () => new OAGC\Strategy\QueryStringScalarArray(['type' => OAGC\Validator\QueryStringScalarArray::TYPE_INT, 'format' => 'test']))->toThrow($exception);
		});
	});
